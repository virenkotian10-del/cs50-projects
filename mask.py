...
def get_color_for_attention_score(attention_score):
    ...
    """
    # Convert attention score to color value (0-255)
...
def visualize_attentions(tokens, attentions):
    ...
    for layer_idx, layer_attentions in enumerate(attentions):
        ...
        batch_attentions = layer_attentions.numpy()
        num_heads = batch_attentions.shape[1]
        
        for head_idx in range(num_heads):
            # Get attention weights for this specific head
            attention_weights = batch_attentions[0][head_idx]
            
            # Generate diagram for this layer and head (using 1-indexed numbering)
            generate_diagram(
                layer_idx + 1,
                head_idx + 1,
                tokens,
